...
for (i in 1:length(my_ids)){
  ...
  if( file.exists(paste(".", my_ids[i], sep="")) != FALSE ){
    ...
    # COMPROMIST - UNIQUE AND CAN GET READABLE NAME EASILY
    unique_rownames <- vector(mode="character")
    for ( j in 1:length(my_ids) ){
      unique_rownames <- c(unique_rownames, paste())
    }
    # Finish making unique name <--------
    ...
    if( i==1 ){ # import first sample data
      ...
      FPKM_matrix <- merge(my_data_temp[, my_dataype], FPKM_matrix, by="row.names", all=TRUE) # This does not handle metadata yet
      ...
      # subloop to add the column names when on the last sample
